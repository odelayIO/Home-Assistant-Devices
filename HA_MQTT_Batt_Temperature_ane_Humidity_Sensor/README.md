...
The firmware reads the WiFi and MQTT broker credentials from `arduino_secrets.h`, which is not stored in the repository.  Before building, rename (or copy) `arduino_secrets.example` to `arduino_secrets.h` in the `XIAO_MQTT_Battery_Small_Wine_Frig/` sketch folder and fill in your credentials:
